...
| Standard mobile hardware | Android `minSdkVersion` is 26, matching Android 8.0+, and the included iOS project targets iOS 15.5 (required by the ML Kit face detector). The native stack uses CPU-capable TFLite and avoids GPU-only assumptions. |
...
1. **iOS floor is 15.5, not 12.** Two constraints stack here: React Native 0.74
   already drops support below iOS 13.4, and the on-device ML Kit face detector
   (GoogleMLKit/FaceDetection) requires **iOS 15.5+**, so the project targets
   15.5. In practice 2026-era mid-range field devices run iOS 15+, so this is a
   non-issue in the field, but if iOS 12 is a hard requirement it would mean
   swapping ML Kit for a lighter-floored detector — a tradeoff to call out, not
   hide.
